...
function Bubble({ role, content }) {
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-3`}>
      {!isUser && (
        // ACCESSIBILITY: Leaf icon is decorative — aria-hidden
        <div
          className="w-7 h-7 bg-eco-600 rounded-full flex items-center justify-center mr-2 shrink-0 mt-0.5"
          aria-hidden="true"
        >
...
export default function Insights() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.error(e);
        if (!cancelled) setAiError(e.message || 'Unknown error occurred.');
      }
  ...
  // EFFICIENCY: useCallback on handleSend
    ...
    <div className="max-w-3xl animate-fade-in">
      ...
      {!hasData && !loading && (
        <div className="bg-white border border-dashed border-gray-300 rounded-2xl p-12 text-center mb-6">
          ...
          <Link
            to="/calculator"
            className="inline-flex items-center gap-2 bg-eco-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-eco-500 transition-all"
          >
            <Calculator size={15} aria-hidden="true" /> Open Calculator
          </Link>
        </div>
      )}
      ...
      {/* Loading */}
      {loading && hasData && (
        <div className="bg-white border border-gray-200 rounded-2xl p-10 text-center mb-6" role="status" aria-label="Loading AI insights">
          <div className="w-10 h-10 mx-auto rounded-full border-4 border-eco-100 border-t-eco-500 animate-spin mb-4" aria-hidden="true" />
      ...
      {/* Error */}
      {aiError && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center mb-6" role="alert">
          ...
          <button
            onClick={regenerate}
            className="bg-red-500 text-white px-5 py-2 rounded-xl text-sm font-semibold hover:bg-red-400 transition-all"
          >
            Retry
          </button>
        </div>
      ...
      {insights && (
        <div className="mb-6">
          <div className="bg-eco-950 rounded-2xl p-5 mb-4 flex items-start gap-3">
            <Lightbulb size={20} className="text-eco-400 shrink-0 mt-0.5" aria-hidden="true" />
            <div>
            ...
            {insights.insights.map((ins) => (
              ...
              >
                <button
                  ...
                  aria-expanded={expanded === ins.id}
                  aria-controls={`insight-body-${ins.id}`}
                  className="w-full text-left px-5 py-4 flex items-center gap-4"
                >
                  <span className="text-2xl shrink-0" aria-hidden="true">{ins.emoji}</span>
                  ...
                  {expanded === ins.id
                    ? <ChevronUp size={16} className="text-gray-400 shrink-0" aria-hidden="true" />
                    : <ChevronDown size={16} className="text-gray-400 shrink-0" aria-hidden="true" />}
                </button>
                {expanded === ins.id && (
                  <div
                    id={`insight-body-${ins.id}`}
                    className="px-5 pb-4 pt-0 border-t border-gray-50"
                  >
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-2">
          <div className="w-6 h-6 bg-eco-600 rounded-full flex items-center justify-center" aria-hidden="true">
            <Leaf size={12} className="text-white" aria-hidden="true" />
          </div>
          <span className="font-semibold text-gray-800 text-sm">Chat with EcoAI</span>
          <span className="bg-eco-50 text-eco-600 text-[10px] font-bold px-2 py-0.5 rounded-full ml-auto" aria-label="Powered by Gemini">
            GEMINI
          </span>
        </div>

        {/* ACCESSIBILITY: aria-live="polite" on message container.
            Screen readers will announce new messages as they stream in
            without interrupting the user mid-sentence. */}
        <div
          className="p-4 min-h-40 max-h-72 overflow-y-auto scrollbar-thin"
          aria-live="polite"
          aria-atomic="false"
          aria-label="EcoAI conversation"
          role="log"
        >
          {messages.length === 0 && (
            <div className="text-center py-4">
              <p className="text-gray-400 text-sm mb-4">Ask EcoAI anything about reducing your footprint</p>
              <div className="flex flex-wrap gap-2 justify-center" role="group" aria-label="Suggested questions">
                {STARTERS.map(s => (
                  <button
                    key={s}
          ...
          {streaming && messages[messages.length - 1]?.content === '' && (
            <div className="flex items-center gap-2 text-gray-400 text-sm pl-10" aria-label="EcoAI is typing" role="status">
              <div className="w-1.5 h-1.5 bg-eco-400 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} aria-hidden="true" />
              <div className="w-1.5 h-1.5 bg-eco-400 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} aria-hidden="true" />
              <div className="w-1.5 h-1.5 bg-eco-400 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} aria-hidden="true" />
            </div>
          )}
          <div ref={bottomRef} />
        </div>

        {/* ACCESSIBILITY: Input has aria-label, send button has aria-label */}
        <div className="px-4 py-3 border-t border-gray-100 flex gap-2">
          <input
            ...
            disabled={streaming}
            aria-label="Ask EcoAI a question about your carbon footprint"
          ...
          <button
            ...
            aria-label="Send message to EcoAI"
